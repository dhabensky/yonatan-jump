...
public class GenerateBlock {
	...
	public Block createBlock() {
		...
		if (rand.nextInt(6) < 1) {
			scene.delayedAddObject(DhabenskyActivity.Companion.createCollectable(resources, rand.nextInt(DhabenskyActivity.Companion.getIds().length), block.getPos().x + block.getW() / 2, block.getPos().y - block.getH()));
		}
		else {
			if (rand.nextInt(3) < 1) {
				scene.delayedAddObject(DhabenskyActivity.Companion.createBurgers(resources, rand.nextInt((int) ((block.getPos().x - block.getPos().x + block.getW()) + 1)) + block.getPos().x + block.getW(), block.getPos().y - block.getH()));
			}
